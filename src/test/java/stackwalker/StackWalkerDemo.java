...
public class StackWalkerDemo {
  ...
  public List<StackFrame> walkExample2(Stream<StackFrame> stackFrameStream) {
    return stackFrameStream.filter(frame -> frame.getClassName().contains("stackwalker"))
        .collect(Collectors.toList());
  }

  public String walkExample3(Stream<StackFrame> stackFrameStream) {
    return stackFrameStream.filter(frame -> frame.getClassName().contains("stackwalker") && frame.getClassName().endsWith("Test"))
        .findFirst()
